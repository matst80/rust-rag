...
pub async fn ingest_url(
    ...
    // 1. Fetch HTML
    let html = if req.use_cdp {
        fetch_with_cdp(&state, &req.url).await?
    } else {
        fetch_with_reqwest(&state, &req.url).await?
    };
    tracing::debug!(html_len = html.len(), "fetched HTML");

    // 2. HTML to Markdown
    let md = html2md::parse_html(&html);
    tracing::debug!(md_len = md.len(), "converted to markdown");
    ...
    let store_req = StoreRequest {
        ...
        chunk: Some(crate::api::ChunkConfig::default()),
        path: req.path,
...
async fn fetch_with_reqwest(state: &AppState, url: &str) -> Result<String, ApiError> {
    ...
    resp.text().await.map_err(|e| ApiError::Internal(anyhow::anyhow!("failed to read response text: {e}")))
}

async fn fetch_with_cdp(state: &AppState, url: &str) -> Result<String, ApiError> {
