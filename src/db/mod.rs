...
pub fn default_ontology_predicates() -> Vec<OntologyPredicateRecord> {
    ...
    vec![
        ...
        OntologyPredicateRecord {
            name: "supersedes".to_owned(),
            source_id: None,
            description: "from replaces, obsoletes, or updates to as the new truth".to_owned(),
            direction: "from replaces/obsoletes to".to_owned(),
            example_from: Some("v2 API spec".to_owned()),
            example_to: Some("v1 API spec".to_owned()),
            created_at: now,
            updated_at: now,
        },
    ]
