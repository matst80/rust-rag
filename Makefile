...
# rust-rag-oauth k8s Secret; here we target the locally-running Next.js
# frontend on :3000.
GOOGLE_OAUTH_REDIRECT_URI ?= http://localhost:3000/api/integrations/google/callback

# Web Push (VAPID). Generated once via:
#   openssl ecparam -name prime256v1 -genkey -noout -out vapid_private.pem
# then derived as base64-url (no padding). Same keys work locally and in
# prod — they identify the *server*, not a user.
VAPID_PUBLIC_KEY ?= BFN9cq-lw4nZJ_SMzkpZyAsp723SmcR4_hghSAJJ53UFkij5jBEEdrcmHp2iubzdT9Wz-mcGHw_JHsVz_KmuJIw
VAPID_PRIVATE_KEY ?=
VAPID_SUBJECT ?= mailto:mats.tornberg@gmail.com
