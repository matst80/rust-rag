...
export function KanbanBoard() {
  ...
  const handleMoveCard = useCallback(
    async (card: Entry, newStatus: string) => {
      const updatedData = {
        ...(card.data as any),
        status: newStatus,
      };
  ...
  if (isLoading)
    return (
      <div className="flex items-center justify-center h-64 font-mono text-sm animate-pulse">
        Loading board…
      </div>
    );
  if (error)
    return (
      <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-md text-destructive flex items-center gap-3">
  ...
  return (
    <>
      {/* Edit/New Dialog */}
      <Dialog
        open={!!editingCard || !!isAddingToColumn}
        onOpenChange={(open) => {
          if (!open) {
            setEditingCard(null);
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0">
          <DialogHeader className="p-6 pb-2">
            <DialogTitle className="font-mono text-sm uppercase tracking-widest">
              {editingCard
                ? "Edit Card"
                : `New Card in ${isAddingToColumn?.replace("_", " ")}`}
            </DialogTitle>
          </DialogHeader>
        ...
        {COLUMNS.map((column) => (
          <div
            ...
            onDrop={() => {
              if (
                draggedCard &&
                (draggedCard.data as any).status !== column.id
              ) {
          ...
          >
            <div className="p-4 flex items-center justify-between border-b border-border/50">
              <div className="flex items-center gap-2">
                <h3 className="font-mono text-[10px] font-black uppercase tracking-[2px]">
                  {column.title}
                </h3>
                <Badge
                  variant="outline"
                  className={cn(
                    "px-1.5 py-0 text-[10px] rounded-sm font-mono",
                    column.color,
                  )}
                >
              ...
              <AnimatePresence initial={false}>
                {cardsByStatus[column.id].map((card) => (
                  <KanbanCard
                    key={card.id}
                    card={card}
                    openEntry={() => router.push(`/entries/${card.id}`)}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="size-7 -mr-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-32 font-mono text-[10px] uppercase tracking-wider"
              >
                <DropdownMenuItem onClick={onEdit} className="gap-2">
                  <Pencil className="size-3" /> Edit
                </DropdownMenuItem>
                <DropdownMenuItem onClick={openEntry} className="gap-2">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {data.priority && (
              <Badge
                variant="outline"
                className={cn(
                  "px-1.5 py-0 text-[9px] rounded-sm font-black uppercase tracking-tighter",
