...
export function StructuredDataEditor({
  ...
  const handleCodeChange = (text: string | undefined) => {
    ...
    try {
      ...
      setValidationErrors(
        isValid
          ? null
          : result.errors.map((e) => (e as any).stack || e.message),
      );
  ...
  const handleFormChange = (
    ...
    setValidationErrors(
      finalErrors.length > 0
        ? finalErrors.map((e) => (e as any).stack || e.message)
        : null,
    );
    onChange?.(data);
  ...
  const handleExtractError = (raw: string) => {
    setJsonText(raw);
    setError(
      "AI produced invalid JSON. You can try to fix it manually in the code editor below.",
    );
    setValidationErrors(null);
    setActiveTab("code");
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        {error && (
          <div className="mt-3 p-3 bg-red-500/10 border border-red-500/20 rounded-md flex items-start gap-3 animate-in slide-in-from-top-1 duration-200 shadow-sm">
            ...
            <div className="space-y-1">
              <div className="text-[10px] font-black text-red-500 uppercase tracking-[2px] font-mono leading-none">
                Syntax Error
              </div>
              <div className="text-[11px] font-mono text-red-400/90 leading-tight">
                {error}
              </div>
            </div>
          </div>
        ...
        {validationErrors && validationErrors.length > 0 && (
          <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-md flex items-start gap-3 animate-in slide-in-from-top-1 duration-200 shadow-sm">
            ...
            <div className="space-y-1">
              <div className="text-[10px] font-black text-amber-500 uppercase tracking-[2px] font-mono leading-none">
                Validation Error
              </div>
              <ul className="list-none p-0 m-0">
                {validationErrors.map((err, idx) => (
                  <li
                    key={idx}
                    className="text-[11px] font-mono text-amber-400/90 leading-tight flex gap-1.5"
                  >
        ...
        <TabsContent value="form" className="mt-3 min-h-[300px]">
          <div className="rjsf-wrapper">
            <Form
              ...
              formData={value || {}}
              onChange={(e) =>
                handleFormChange(e.formData, e.errors.length === 0, e.errors)
              }
        ...
        <TabsContent
          value="code"
          className="mt-3 border rounded-md overflow-hidden bg-card"
        >
          <Editor
            height="400px"
      ...
      <style jsx global>{`
        ...
        .rjsf-wrapper input:focus {
          outline: none;
          ring: 2px;
          ring-color: var(--primary);
          border-color: var(--primary);
        }
        .rjsf-wrapper .help-block {
          font-size: 0.75rem;
          color: var(--muted-foreground);
          margin-top: 0.25rem;
        }
        ...
        .rjsf-wrapper .array-item > div:first-child {
          flex: 1;
        }
        .rjsf-wrapper .array-item-toolbox {
          display: flex;
          gap: 4px;
          padding-bottom: 6px;
        }
        .rjsf-wrapper .btn-group {
          display: flex;
          gap: 2px;
          padding-inline: 5px;
        }
        ...
        .rjsf-wrapper .btn-group button i {
          display: none;
        }
        .rjsf-wrapper .rjsf-array-item-move-up::after {
          content: "↑";
          font-size: 12px;
        }
        .rjsf-wrapper .rjsf-array-item-move-down::after {
          content: "↓";
          font-size: 12px;
        }
        .rjsf-wrapper .btn-group button:hover {
          background: var(--primary);
          color: var(--primary-foreground);
        }
        .rjsf-wrapper .btn-group button:disabled {
          opacity: 0.3;
          cursor: not-allowed;
        }

        .rjsf-wrapper .error-detail {
