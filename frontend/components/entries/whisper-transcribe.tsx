...
export function WhisperTranscribe({ onTranscription }: WhisperTranscribeProps) {
  ...
  const { data: whisperOverviewEntry } = useItem(
    "projects_whisper_slask_overview",
  );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size="sm"
            className="gap-2 h-10 px-2 transition-all duration-300"
        ...
        <TooltipContent side="top">
          <p className="text-xs">
            {isRecording
              ? "Stop recording"
              : `Transcription via ${whisperWsUrl}`}
          </p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
