...
export function AgentChat() {
	...
	const [terminalFullScreen, setTerminalFullScreen] = useState<Record<string, boolean>>({})
	const [selectedCommand, setSelectedCommand] = useState<Record<string, string | null>>({})

	const active = activeSessionId ? sessions[activeSessionId] : null
	const draft = activeSessionId ? drafts[activeSessionId] ?? "" : ""
	const activeSelectedCommand = activeSessionId ? selectedCommand[activeSessionId] ?? null : null
	...
	const sendPrompt = useCallback(() => {
		if (!activeSessionId || (!draft.trim() && !selectedCommand[activeSessionId]) || conn.status !== "open") return
	...
	const createTerminal = useCallback((sid: string) => {
	...
	const closeTerminal = useCallback((tid: string) => {
		send({
			type: "close_terminal",
			terminal_id: tid,
		})
	...
	const onTerminalInput = useCallback((tid: string, data: string) => {
		send({
			type: "terminal_input",
			terminal_id: tid,
			data,
	...
	const onTerminalResize = useCallback((tid: string, cols: number, rows: number) => {
		send({
	...
	const onTerminalAttach = useCallback((tid: string, cols: number, rows: number) => {
		send({
					...
					<div className="flex-1 flex flex-col min-h-0 overflow-hidden">
						...
						<ResizablePanelGroup direction="vertical" className="flex-1 min-h-0">
							...
							<ResizablePanel 
								...
								className={cn(
									"flex flex-col",
									activeSessionId && terminalFullScreen[activeSessionId] && "hidden"
								)}
							>
								<div
							...
							{/* TERMINAL HANDLE */}
							{activeSessionId && viewMode[activeSessionId] === "terminal" && activeTerminalId[activeSessionId] && !terminalFullScreen[activeSessionId] && (
								<ResizableHandle withHandle />
							)}

							{/* TERMINAL SECTION (Resizable Middle section) */}
							{activeSessionId && viewMode[activeSessionId] === "terminal" && activeTerminalId[activeSessionId] && (
								<ResizablePanel 
								...
								>
									<div className="flex flex-col h-full bg-muted/5">
										{/* TERMINAL TABS */}
										{sessionTerminals[activeSessionId] && sessionTerminals[activeSessionId].length > 0 && (
											<div className="flex items-center gap-1 border-b border-border bg-muted/20 px-4 py-1.5 overflow-x-auto no-scrollbar shrink-0">
												...
												<button
													...
													title={terminalFullScreen[activeSessionId] ? "Exit Full Screen" : "Full Screen"}
												>
							...
							<div className="border-t border-border bg-amber-500/5 p-4 shrink-0">
								...
								<div className="space-y-2 max-h-40 overflow-y-auto">
									{pendingForActive.map((p) => (
										<div key={p.localSeq} className="text-sm p-3 bg-card border border-border rounded-md shadow-sm font-mono text-[10px]">
						...
						<div className="shrink-0 flex flex-col border-t border-border bg-background">
							{active?.available_commands && active.available_commands.length > 0 && (
								<div className="flex items-center gap-2 px-4 py-2 bg-muted/5 overflow-x-auto no-scrollbar scroll-smooth">
									...
									{active.available_commands.map((cmd) => (
										<button
											...
											className={cn(
												"inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-medium transition-all shadow-sm active:scale-95 whitespace-nowrap",
							...
							>
								<div className="flex items-end gap-3 rounded-lg border border-input bg-background p-2 focus-within:border-primary/50 transition-colors shadow-sm">
									...
									<div className="flex items-center gap-2 mb-1 shrink-0">
										...
										<button
											type="submit"
											disabled={(!draft.trim() && !activeSelectedCommand) || conn.status !== "open"}
											className={cn(
												"flex size-10 items-center justify-center rounded-md transition-all shadow-sm",
												(draft.trim() || activeSelectedCommand) && conn.status === "open"
