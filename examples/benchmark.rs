...
async fn main() -> Result<()> {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        match ort::ep::CUDA::default().is_available() {
            Ok(true) => {
                println!("CUDA Execution Provider is supported by this ONNX Runtime build.")
            }
            Ok(false) => println!(
                "CUDA Execution Provider is NOT supported by this ONNX Runtime build! (Fallback to CPU may occur silently)"
            ),
...
fn execution_providers() -> Vec<ort::execution_providers::ExecutionProviderDispatch> {
    #[cfg(feature = "cuda")]
    {
        use ort::ep::{ArenaExtendStrategy, CPUExecutionProvider, CUDA, cuda::ConvAlgorithmSearch};
        ...
        println!(
            "Initializing with CUDA Execution Provider (Device ID: {device_id}, Memory Limit: {mem_limit_mb} MiB)..."
        );
        ...
        vec![
            cuda.with_arena_extend_strategy(ArenaExtendStrategy::SameAsRequested)
...
fn benchmark_model(path: &Path, name: &str) -> Result<()> {
    ...
    for _ in 0..3 {
        let _ = run_inference(
            ...
            &input_ids,
            &attention_mask,
            has_token_type.then_some(&token_type_ids),
        )?;
    }
